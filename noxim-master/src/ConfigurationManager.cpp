...
void loadConfiguration() {
    ...
    //NEW, read from YAML file with default of false
    GlobalParams::use_optinoc = readParam<bool>(config, "use_optinoc", false);
...
void showHelp(char selfname[])
{
    cout << "Usage: " << selfname << " [options]" << endl
	 ...
	 << "\t-vc N\t\t\tNumber of virtual channels" << endl
         << "\t-optinoc\t\tEnable optical communication in NOC" << endl
...
void checkConfiguration()
{
	...
	{
		//NEW, error when optical enabled for topologies other than mesh
        if(GlobalParams::use_optinoc)
        {
            cerr << "Error: Optical NOC is implemented only for Mesh topology" << endl;
            exit(1);
        }
    ...
    //NEW, prevent both wireless and optical flags being enabled together
    if(GlobalParams::use_optinoc && GlobalParams::use_winoc)
    {
        cerr << "Error: both wireless and optical cannot be enabled simultaneously" << endl;
        exit(1);
    }
...
void parseCmdLine(int arg_num, char *arg_vet[])
{
	...
	for (int i = 1; i < arg_num; i++) 
	{
        ...
        else if (!strcmp(arg_vet[i], "-optinoc"))
        GlobalParams::use_optinoc = true;
