...
double Stats::totalHops(){
	double sum = 0.0;

	for(int i = 0;i<chist.size();i++){
		for(auto it : chist[i].hops){
			sum+=it;
		}	
	}
	return sum;
...
// //new --- trying to track the delay from tail flit reception
...
// 	chist[i].delays.push_back(arrival_time - flit.timestamp);
	...
	//new
	if (flit.flit_type == FLIT_TYPE_ACK)
	{
		chist[i].req_ack_delays.push_back(arrival_time - flit.timestamp);
		// cout<<chist[i].req_ack_delays.size()<<endl;
		chist[i].hops.push_back(flit.hop_no);
	}
	if(flit.flit_type == FLIT_TYPE_DES)
		chist[i].destroy_delays.push_back(arrival_time - flit.timestamp);
}

double Stats::getAverageReqAckDelay(const int src_id)
{
	double sum = 0.0;
	...
	for (unsigned int j = 0; j < chist[i].req_ack_delays.size(); j++)
	sum += chist[i].req_ack_delays[j];

	return sum / (double) chist[i].req_ack_delays.size();
}

double Stats::getAverageReqAckDelay()
{
	double avg = 0.0;

	for (unsigned int k = 0; k < chist.size(); k++) {
	unsigned int samples = chist[k].req_ack_delays.size();
	if (samples)
		avg += (double) samples *getAverageReqAckDelay(chist[k].src_id);
	}

	return avg / (double) getReceivedPackets();
}

double Stats::getMaxReqAckDelay(const int src_id)
{
	double maxd = -1.0;
	...
	for (unsigned int j = 0; j < chist[i].req_ack_delays.size(); j++)
	if (chist[i].req_ack_delays[j] > maxd) {
		maxd = chist[i].req_ack_delays[j];
	}
	return maxd;
}

double Stats::getMaxReqAckDelay()
{
	double maxd = -1.0;
	...
	unsigned int samples = chist[k].req_ack_delays.size();
	if (samples) {
		double m = getMaxReqAckDelay(chist[k].src_id);
		if (m > maxd)
		maxd = m;
	}
	}
...
double Stats::getAverageDestroyDelay(const int src_id)
{
	double sum = 0.0;
	...
	for (unsigned int j = 0; j < chist[i].destroy_delays.size(); j++)
	sum += chist[i].destroy_delays[j];

	return sum / (double) chist[i].destroy_delays.size();
}

double Stats::getAverageDestroyDelay()
{
	double avg = 0.0;

	for (unsigned int k = 0; k < chist.size(); k++) {
	unsigned int samples = chist[k].destroy_delays.size();
	if (samples)
		avg += (double) samples *getAverageDestroyDelay(chist[k].src_id);
	}

	return avg / (double) getReceivedPackets();
}

double Stats::getMaxDestroyDelay(const int src_id)
{
	double maxd = -1.0;
	...
	for (unsigned int j = 0; j < chist[i].destroy_delays.size(); j++)
	if (chist[i].destroy_delays[j] > maxd) {
		maxd = chist[i].destroy_delays[j];
	}
	return maxd;
}

double Stats::getMaxDestroyDelay()
{
	double maxd = -1.0;
	...
	unsigned int samples = chist[k].destroy_delays.size();
	if (samples) {
		double m = getMaxDestroyDelay(chist[k].src_id);
		if (m > maxd)
		maxd = m;
	}
	}
