...
struct CommHistory {
    ...
    // req ack delay
    vector < double >req_ack_delays;
    // destroy delay
    vector < double >destroy_delays;
    vector <double> hops;
};
...
class Stats {
  ...
  public:
    ...
    // Returns tStatse average throughput (flits/cycle) for the current node
    ...
    // for req ack delay
    double getAverageReqAckDelay(const int src_id);
    double getMaxReqAckDelay(const int src_id);

    double getAverageReqAckDelay();
    double getMaxReqAckDelay();

    // for destroy delay
    double getAverageDestroyDelay(const int src_id);
    double getMaxDestroyDelay(const int src_id);
