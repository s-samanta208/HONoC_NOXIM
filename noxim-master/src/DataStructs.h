...
enum FlitType {
    FLIT_TYPE_HEAD, FLIT_TYPE_BODY, FLIT_TYPE_TAIL, FLIT_TYPE_REQ, FLIT_TYPE_ACK, FLIT_TYPE_DES
};
...
struct Flit {
    ...
    FlitType flit_type;	// The flit type (FLIT_TYPE_HEAD, FLIT_TYPE_BODY, FLIT_TYPE_TAIL or control flitTypes)
    ...
    inline bool isEquivalent(const Flit & flit) const {
	return (flit.src_id == src_id && flit.dst_id == dst_id
		&& flit.vc_id == vc_id
		&& flit.sequence_length == sequence_length);
    }
...
//Added one enum entry for OpticalMesh - OPTI_INTERFACE_PWR_D
enum
{
    BUFFER_PUSH_PWR_D,
    BUFFER_POP_PWR_D,
    ...
    WIRELESS_SNOOPING,
    OPTI_INTERFACE_PWR_D,
    NO_BREAKDOWN_ENTRIES_D
};

//added static enum entry MR_ONSTATE_PWR_S related to OpticalMesh
enum
{
