...
//Build optical mesh function.
void NoC::buildOpticalMesh()
{
	int total_elems = GlobalParams::mesh_dim_x * GlobalParams::mesh_dim_y;
	
    //Creating incoming signal for each optical router. First create dimX row array
    flit_opti = new sc_signal<Flit>*[GlobalParams::mesh_dim_x+1];

	//for each row, create dimY number of signals 
    for (int i = 0; i < GlobalParams::mesh_dim_x+1; i++) 
	{
        flit_opti[i] = new sc_signal<Flit>[GlobalParams::mesh_dim_y+1];
    }

	// Create MESH of Optical Routers pointers
    opti = new OptiRouter**[GlobalParams::mesh_dim_x];
    for (int i = 0; i < GlobalParams::mesh_dim_x; i++) 
	{
    	opti[i] = new OptiRouter*[GlobalParams::mesh_dim_y];
    }

    // Create the mesh of Optical Routers
    for (int j = 0; j < GlobalParams::mesh_dim_y; j++) 
	{
		for (int i = 0; i < GlobalParams::mesh_dim_x; i++) 
		{
			...
			opti_coord.x = i;
			opti_coord.y = j;
			int opti_id = coord2Id(opti_coord);
			sprintf(opti_name, "Opti[%02d][%02d]_(#%d)", i, j, opti_id);
			opti[i][j] = new OptiRouter(opti_name);

			//Configure Optical router by providing the coordinates
			opti[i][j]->configure(j * GlobalParams::mesh_dim_x + i,
					GlobalParams::stats_warm_up_time,
					GlobalParams::buffer_depth);
			opti[i][j]->power.configureRouter(GlobalParams::flit_size,
							GlobalParams::buffer_depth,
							GlobalParams::flit_size,
							string(GlobalParams::routing_algorithm),
							"default");
			...
			// Map clock and reset
			opti[i][j]->clock(clock);
			opti[i][j]->reset(reset);
			
			// Map Rx signals
			// opti[i][j]->flit_optical_rx(flit_opti[i][j]);
			
			//new
			opti[i][j]->flit_optical_rx[DIRECTION_NORTH] (flit_opti[i][j]);
			opti[i][j]->flit_optical_rx[DIRECTION_EAST] (flit_opti[i+1][j]);
			opti[i][j]->flit_optical_rx[DIRECTION_SOUTH] (flit_opti[i][j+1]);
			opti[i][j]->flit_optical_rx[DIRECTION_WEST] (flit_opti[i][j]);

			// Map Tx signals. Array where index denotes destination router id//
			
			//new
			for(int w=0;w<4;w++){
				opti[i][j]->flit_optical_tx[w] = new sc_out<Flit> [total_elems];
			}

			// opti[i][j]->flit_optical_tx = new sc_out<Flit> [total_elems];

			//initialise all the transmission ports of current optical router (dimX*dimY ports)
			for (int l = 0; l < GlobalParams::mesh_dim_y; l++)
			{
				for (int k = 0; k < GlobalParams::mesh_dim_x; k++)
				{
					int dest_id = l * GlobalParams::mesh_dim_x + k;

					// opti[i][j]->flit_optical_tx[dest_id] (flit_opti[k][l]);
					
					//new
					opti[i][j]->flit_optical_tx[0][dest_id] (flit_opti[k][l]);
					opti[i][j]->flit_optical_tx[1][dest_id] (flit_opti[k][l+1]);
					opti[i][j]->flit_optical_tx[2][dest_id] (flit_opti[k+1][l]);
					opti[i][j]->flit_optical_tx[3][dest_id] (flit_opti[k+1][l+1]);
...
void NoC::buildMesh()
{
	...
	for (int i = 0; i < GlobalParams::mesh_dim_x; i++) {
	    ...
	    // signals/port when tile receives(rx) from hub/optical mesh
	    t[i][j]->hub_req_rx(req[i][j].from_hub);
	    t[i][j]->hub_flit_rx(flit[i][j].from_hub);
	    t[i][j]->hub_ack_rx(ack[i][j].to_hub);
	    t[i][j]->hub_buffer_full_status_rx(buffer_full_status[i][j].to_hub);

	    // signals/port when tile transmits(tx) to hub/optical mesh
	    t[i][j]->hub_req_tx(req[i][j].to_hub); // 7, sc_out
	    t[i][j]->hub_flit_tx(flit[i][j].to_hub);
	    t[i][j]->hub_ack_tx(ack[i][j].from_hub);
	    t[i][j]->hub_buffer_full_status_tx(buffer_full_status[i][j].from_hub);
		...
		//Assigning the same signals to corresponding hubs
		map<int, int>::iterator it = GlobalParams::hub_for_tile.find(tile_id);
		if (it != GlobalParams::hub_for_tile.end())
		{
			int hub_id = GlobalParams::hub_for_tile[tile_id];
		...
		if(GlobalParams::use_optinoc)
		{
			// Assign the signals to corresponding optical routers
			//signals for recieving process of the optical routers from electrical mesh
			opti[i][j]->req_electrical_rx (req[i][j].to_hub);
			opti[i][j]->flit_electrical_rx(flit[i][j].to_hub);
			opti[i][j]->ack_electrical_rx (ack[i][j].from_hub);
			opti[i][j]->buffer_full_status_rx (buffer_full_status[i][j].from_hub);

			//signals for transmitting process of the optical routers to electrical mesh
			opti[i][j]->req_electrical_tx (req[i][j].from_hub);
			opti[i][j]->flit_electrical_tx (flit[i][j].from_hub);
			opti[i][j]->ack_electrical_tx (ack[i][j].to_hub);
			opti[i][j]->buffer_full_status_tx (buffer_full_status[i][j].to_hub);
		}
