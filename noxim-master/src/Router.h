...
SC_MODULE(Router)
{
    ...
    //NEW
    bool opticalReserve[DIRECTIONS+1];    //used to reserve an output direction of the optical router
    bool reqSent;                        //indicate if req control flit sent for top flit in local buffer
    bool ackReceived;                //indicate if ack control flit received for top flit in local buffer
    queue< pair<Flit,int> > desFlitQ;     //queue which stores destroy control flit as well as time to send
    int numOnStateMR;               //stores the current number of MRs in ON state for current router

    //New buffer created to store ACK control flit for each router. This is to prevent deadlocking
    //which would occur when REQ flit and ACK flit stored in same buffer. REQ flit might be blocked
    //but this would result ACK flit to be blocked as well which results in overall deadlock.
    //By doing this, ACK flit will never be blocked
    BufferBank buffer_ack;      
