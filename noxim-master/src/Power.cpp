...
Power::Power()
{
    ...
    //NEW related to optical mesh
    opti_interface_pwr_d = 0.0;
    mr_onstate_pwr_s = 0.0;
...
// (s)tatic, (d)ynamic power
    ...
    //NEW, initialising values of power for OpticalMesh. HARDCODING!!!!
    
    //OE and EO conversion requires 1pJ/bit. multiply it by flitSize to get energy per flit in joules
    opti_interface_pwr_d=GlobalParams::flit_size*(1e-12);
    //each MR in onstate consumes 20uW of power. we convert into joules by multiplying with clock period
    mr_onstate_pwr_s=W2J(20e-6);
}
...
//NEW function. Called each time O/E or E/O conversion of flit takes place
void Power::opticalInterface()
{
    power_dynamic.breakdown[OPTI_INTERFACE_PWR_D].value += opti_interface_pwr_d;
}

//called during each cycle with number of MRs switched on as a parameter
void Power::mrOnState(int numMRs)
{
    if(numMRs<0)
    {
        assert(false);
    ...
    power_static.breakdown[MR_ONSTATE_PWR_S].value += (numMRs * mr_onstate_pwr_s);
}
...
void Power::initPowerBreakdown()
{
    ...
    //NEW dynamic power breakdown entry added related to optical Mesh
    initPowerBreakdownEntry(&power_dynamic.breakdown[OPTI_INTERFACE_PWR_D], "opti_interface_pwr_d");
    ...
    //NEW static power breakdown entry added related to optical mesh
    initPowerBreakdownEntry(&power_static.breakdown[MR_ONSTATE_PWR_S],"mr_onstate_pwr_s");
}
