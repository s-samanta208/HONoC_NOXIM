...
double GlobalStats::getTotalHops(){
	double sum = 0.0;
	for (int y = 0; y < GlobalParams::mesh_dim_y; y++){
	    for (int x = 0; x < GlobalParams::mesh_dim_x; x++){
			sum+=noc->t[x][y]->r->stats.totalHops();
		}
	}
	return sum;
}
double GlobalStats::getAverageReqAckDelay(){
	unsigned int total_packets = 0;
	double avg_delay_req_ack = 0.0;
	...
	for (int y = 0; y < GlobalParams::mesh_dim_y; y++)
		...
		if (received_packets) 
		{
		    avg_delay_req_ack +=
			received_packets *
			noc->t[x][y]->r->stats.getAverageReqAckDelay();
		    total_packets += received_packets;
		}
	    }
	}
	...
	for (int y = 0; y < GlobalParams::n_delta_tiles; y++)
	{
		...
		avg_delay_req_ack +=
		    received_packets *
		    noc->core[y]->r->stats.getAverageReqAckDelay();
		total_packets += received_packets;
	    }
	}

	}
	avg_delay_req_ack /= (double) total_packets;

    return avg_delay_req_ack;
}

double GlobalStats::getAverageDestroyDelay(){
	unsigned int total_packets = 0;
	double avg_delay_destroy = 0.0;
	...
	for (int y = 0; y < GlobalParams::mesh_dim_y; y++)
		...
		if (received_packets) 
		{
		    avg_delay_destroy +=
			received_packets *
			noc->t[x][y]->r->stats.getAverageDestroyDelay();
		    total_packets += received_packets;
		}
	    }
	}
	...
	for (int y = 0; y < GlobalParams::n_delta_tiles; y++)
	{
		...
		avg_delay_destroy +=
		    received_packets *
		    noc->core[y]->r->stats.getAverageDestroyDelay();
		total_packets += received_packets;
	    }
	}

	}
	avg_delay_destroy /= (double) total_packets;
	return avg_delay_destroy;
}
...
double GlobalStats::getAverageReqAckDelay(const int src_id,
					 const int dst_id)
{
	Tile *tile = noc->searchNode(dst_id);
	...
	return tile->r->stats.getAverageReqAckDelay(src_id);
}

double GlobalStats::getAverageDestroyDelay(const int src_id,
					 const int dst_id)
{
	Tile *tile = noc->searchNode(dst_id);
	...
	return tile->r->stats.getAverageDestroyDelay(src_id);
}
...
double GlobalStats::getMaxReqAckDelay()
{
	double maxd = -1.0;
	...
	for (int y = 0; y < GlobalParams::mesh_dim_y; y++)
		...
		coord.y = y;
		int node_id = coord2Id(coord);
		double d = getMaxReqAckDelay(node_id);
		if (d > maxd)
		    maxd = d;
	    }
...
double GlobalStats::getMaxDestroyDelay()
{
	double maxd = -1.0;
	...
	for (int y = 0; y < GlobalParams::mesh_dim_y; y++)
		...
		coord.y = y;
		int node_id = coord2Id(coord);
		double d = getMaxDestroyDelay(node_id);
		if (d > maxd)
		    maxd = d;
	    }
...
double GlobalStats::getMaxReqAckDelay(const int node_id)
{
	if (GlobalParams::topology == TOPOLOGY_MESH) 
	{
	Coord coord = id2Coord(node_id);
	...
	if (received_packets)
	    return noc->t[coord.x][coord.y]->r->stats.getMaxReqAckDelay();
	else
	    return -1.0;
	}
	else // other delta topologies
	...
	unsigned int received_packets =
	    noc->core[node_id]->r->stats.getReceivedPackets();
	if (received_packets)
	    return noc->core[node_id]->r->stats.getMaxReqAckDelay();
	else
	    return -1.0;
	}
...
double GlobalStats::getMaxDestroyDelay(const int node_id)
{
	if (GlobalParams::topology == TOPOLOGY_MESH) 
	{
	Coord coord = id2Coord(node_id);
	...
	if (received_packets)
	    return noc->t[coord.x][coord.y]->r->stats.getMaxDestroyDelay();
	else
	    return -1.0;
	}
	else // other delta topologies
	...
	unsigned int received_packets =
	    noc->core[node_id]->r->stats.getReceivedPackets();
	if (received_packets)
	    return noc->core[node_id]->r->stats.getMaxDestroyDelay();
	else
	    return -1.0;
	}
...
double GlobalStats::getMaxReqAckDelay(const int src_id, const int dst_id)
{
	Tile *tile = noc->searchNode(dst_id);

	assert(tile != NULL);

	return tile->r->stats.getMaxReqAckDelay(src_id);
}

double GlobalStats::getMaxDestroyDelay(const int src_id, const int dst_id)
{
	Tile *tile = noc->searchNode(dst_id);

	assert(tile != NULL);

	return tile->r->stats.getMaxDestroyDelay(src_id);
}
...
void GlobalStats::showStats(std::ostream & out, bool detailed)
{
    ...
    out << endl;
#endif
	double totalOpticalEnergy = (getTotalHops()*RCUmul + getReceivedPackets()*EtoOPCost*2)*(1e-12);
	double totalEnergy = totalOpticalEnergy+getTotalPower();
    //int total_cycles = GlobalParams::simulation_time - GlobalParams::stats_warm_up_time;
	...
	out << "% Electrical to optical cost (pJ) : " << getReceivedPackets()*EtoOPCost*2<< endl;
	out << "% RCU cost (pJ) : " << double(getTotalHops()*RCUmul)<<endl;
	out << "% Total Optical Energy (J) : " << totalOpticalEnergy<<endl;
	out << "% Total Energy (Optical + Electrical)(J) : " << totalEnergy<<endl;
	out << "%ReqAckDelay:"<<getAverageReqAckDelay()<<endl;
	out << "%DestroyDelay:"<<getAverageDestroyDelay()<<endl;
	out<<"%MaxReqAckDelay:"<<getMaxReqAckDelay()<<endl;
	out<<"%MaxDestroyDelay:"<<getMaxDestroyDelay()<<endl;
